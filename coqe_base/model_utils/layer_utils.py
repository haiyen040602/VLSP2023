...
########################################################################################################################
## làm thế nào dể CRF biết cần phải xác định thực thể nào???
## norm_id_map = num_classes
class CRFCell(nn.Module):
    ...
    def forward(self, feature, mask, target):
        ...
        # CRF sẽ trả về chuỗi num_classes là index của các từ có khả năng là tag nhất 
        if target is None:
        ...
        # compute the conditional log likelihood (loss) of a sequence of tags given emission scores.
        ## emission score = input feature (batch_size, seq_length), tag = target- truth index of element, mask is same length with tag, only use with batch_first
        else:
            output = self.crf(feature, target, mask, reduction='mean')
            output = output.neg() ## [batch_size]: extracted loss of each sequence
        return output 
